...
def create_app(test_config=None):
  ...
  app = Flask(__name__)
  setup_db(app)
  CORS(app, recourses={r"/api/*": {"origins": "*"}})
  ...
  def get_by_category(id):
    ...
    return jsonify({
      'success': True,
      'questions': current_questions,
      'currentCategory': {category.id: category.type},
  ...
  def play_quiz():
    try:
      ...
      if quiz_category['id'] == 0:
        ...
        possible_questions = Question.query.filter(Question.category == quiz_category['id'], Question.id.notin_((previous_questions))).all()

      random_number = random.randrange(len(possible_questions))
      unformatted_question = possible_questions[random_number]
      current_question = unformatted_question.format()
